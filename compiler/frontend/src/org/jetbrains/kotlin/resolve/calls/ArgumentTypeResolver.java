...
public class ArgumentTypeResolver {
    ...
    private void convertSignedConstantToUnsigned(
        ...
        if (constant == null || !constantCanBeConvertedToUnsigned(constant)) return;

        Object constantValue = constant.getValue(TypeUtils.NO_EXPECTED_TYPE);
        if (!(constantValue instanceof Number)) return;

        long value = ((Number) constantValue).longValue();

        IntegerValueTypeConstructor constructor = new IntegerValueTypeConstructor(value, moduleDescriptor, true, false);

        KotlinType newType = KotlinTypeFactory.simpleTypeWithNonTrivialMemberScope(
                Annotations.Companion.getEMPTY(), constructor, Collections.emptyList(), false,
                ErrorUtils.createErrorScope("Scope for number value type: " + constructor, true)
        );

        updateResultArgumentTypeIfNotDenotable(context.trace, context.statementFilter, context.expectedType, newType, expression);
    }
